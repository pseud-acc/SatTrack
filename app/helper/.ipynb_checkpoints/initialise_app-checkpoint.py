...
from app.styles.styles_sat_visualisations import colorscale

# satellite geometry functions
...
def initialise_3d_ls(df, img):
    ...
    surf_3d = go.Surface(x=x, y=y, z=z,
                         ...
                         hoverinfo="none",
                         lighting=dict(
                             ambient=0.6,  # Increase ambient light to reduce harsh contrasts
                             diffuse=0.8,  # Reduce diffuse to soften texture transitions
                             fresnel=0.2,  # Add subtle fresnel effect
                             roughness=0.5,  # Medium roughness for natural appearance
                             specular=0.2  # Reduce specular to minimize glare
                         ),
                         contours=dict(
                             x=dict(highlight=False),  # Turn off contour highlights
